...
class ApplePay
{
    ...
    public function applepay_cancel_order()
    {
        ...
        if (!empty($payment_id)) {
            ...
            $payment = \Payplug\Payment::retrieve($payment_id, new Payplug('yes' === $options['mode'] ? $payplug_live_key : $payplug_test_key));
            $payment->abort(new Payplug('yes' === $options['mode'] ? $payplug_live_key : $payplug_test_key));
        }
