...
class PaymentMethods
{
    ...
    public function payment_method_standard()
    {
        ...
        switch ($payment_method) {
            case 'popup' : $method['popup'] = true;

break;

            case 'integrated' : $method['integrated'] = true;

break;

            default: $method['redirect'] = true;

break;
        }

        $active = (empty($options['payplug'])) || ('yes' === $options['payplug']);
    ...
    public function one_click_option($active = null)
    {
        ...
        if (isset($this->options['oneclick'])) {
            if ('no' != $this->options['oneclick']) {
                $option = $option + ['checked' => true];
            } elseif ('no' == $this->options['oneclick']) {
    ...
    public static function payment_method_applepay($active, $options, $carriers = [])
    {
        ...
        if (!empty($options['applepay_checkout']) && ('yes' == $options['applepay_checkout'])) {
            $checkout = true;
        }

        if (!empty($options['applepay_cart']) && ('yes' == $options['applepay_cart'])) {
            $cart = true;
        }

        if (!empty($options['applepay_product']) && ('yes' == $options['applepay_product'])) {
            $product = true;
        }

        if (true === $active && !$checkout && !$cart) {
            $checkout = true;
        }
