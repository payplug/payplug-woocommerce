...
class WoocommerceActions {
	...
	public function add_payment_retrieve_action( $actions = [] ) {
		...
		if (!in_array($payment_method, ['oney_x3_with_fees', 'oney_x4_with_fees', 'payplug', 'american_express', 'bancontact', 'applepay']) || empty($transaction_id)) {
			return $actions;
		}
	...
	public function handle_payment_retrieve_action( $order ) {
		...
		if (!in_array($payment_method, ['oney_x3_with_fees', 'oney_x4_with_fees', 'payplug', 'american_express', 'bancontact', 'applepay'])) {
			return;
		}
