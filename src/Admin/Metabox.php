		...
		<?php } else { ?>
			<ul>
				...
				<li><span><?php _e('Status', 'payplug'); ?>
						:</span>
					<?php if (isset($metadata['refunded']) && true === $metadata['refunded']) {
            _e('Refunded', 'payplug'); ?>
					<?php
        } elseif (isset($metadata['refunded']) && false === $metadata['refunded'] && (int) $metadata['amount_refunded'] > 0) {
            _e('Partially refunded', 'payplug'); ?>
					<?php
        } elseif (isset($metadata['paid']) && true === $metadata['paid']) {
            _e('Paid', 'payplug'); ?>
					<?php
        } else {
            _e('Not Paid', 'payplug'); ?>
				...
				<?php if (empty($metadata['card_brand']) && 'payplug' != $order->get_payment_method()) { ?>
