...
class ApplePay extends PayplugGateway
{
    ...
    public function __construct()
    {
        ...
        if ($this->checkApplePay() && is_admin()) {
            $this->enabled = 'yes';
        } elseif ($this->checkApplePay() && $this->isSSL()) {
    ...
    public function process_admin_options()
    {
        ...
        if (isset($data['woocommerce_payplug_mode'])) {
            if ('0' === $this->get_post_data()['woocommerce_payplug_mode']) {
        ...
        if (isset($data['woocommerce_payplug_apple_pay'])) {
            if ((1 == $data['woocommerce_payplug_apple_pay']) && (!$this->checkApplePay())) {
    ...
    public function checkApplePay()
    {
        ...
        //it's disabled
        if (isset($options['apple_pay']) && 'no' === $options['apple_pay']) {
            return false;
        }
        ...
        if (!isset($options['applepay_checkout']) && !isset($options['applepay_cart']) && !isset($options['applepay_product']) && isset($options['apple_pay']) && 'yes' === $options['apple_pay']) {
            $this->set_button_checkout(true);
        }

        if (isset($options['applepay_checkout']) && 'yes' === $options['applepay_checkout']) {
            $this->set_button_checkout(true);
        }

        if (isset($options['applepay_cart']) && 'yes' === $options['applepay_cart']) {
            $this->set_button_cart(true);
        }

        if (isset($options['applepay_product']) && 'yes' === $options['applepay_product']) {
    ...
    public function add_apple_pay_js()
    {
        ...
        wp_enqueue_script(
            'payplug-apple-pay',
            PAYPLUG_GATEWAY_PLUGIN_URL . 'assets/js/payplug-apple-pay.js',
            [
                'jquery',
                'apple-pay-sdk',
            ],
