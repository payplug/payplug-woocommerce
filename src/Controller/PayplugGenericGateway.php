...
class PayplugGenericGateway extends PayplugGateway implements PayplugGatewayBuilder
{
    ...
    public function checkGateway()
    {
        ...
        if (empty($options) || empty($options[$this->id]) || 'no' === $options[$this->id]) {
            return false;
        }
    ...
    public function process_refund($order_id, $amount = null, $reason = '')
    {
        ...
        if (isset($this->enable_refund) && false === $this->enable_refund) {
    ...
    {
        if ($this->id === $order->get_payment_method() && isset($this->enable_refund) && false === $this->enable_refund) {
