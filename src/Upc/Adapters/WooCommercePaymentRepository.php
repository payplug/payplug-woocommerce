...
class WooCommercePaymentRepository implements IPaymentRepository
{
    ...
    {
        // Must match on either identity PaymentCaptureOutcomeApplier::persist_operation() may have
        // stored (see Front\UpcWebhook), or a notification bound to the alt id would find no
        // order here at all.
        $orders = wc_get_orders([
            'meta_query' => [
                'relation' => 'OR',
                [
                    'key' => '_payplug_upc_operation_id',
                    'value' => $operationId,
                ],
                [
                    'key' => '_payplug_upc_operation_id_alt',
                    'value' => $operationId,
                ],
            ],
            'limit' => 1,
