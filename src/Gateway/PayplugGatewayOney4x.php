...
class PayplugGatewayOney4x extends PayplugGatewayOney3x
{
    ...
    public function get_icon()
    {
        if ($this->check_oney_is_available() === true) {
	        ...
	        $total_price_oney = $total_price + $this->oney_response['x4_with_fees']['nominal_annual_percentage_rate'];
	        $disable='';
            $f = function ($fn) {
                return $fn;
            };
            if(is_array($this->oney_response)) {
                ...
                <p>
                   ...
                   <div class="payplug-oney-flex">
					<small>( {$f(__('oney_financing_cost', 'payplug'))} <b>{$this->oney_response['x4_with_fees']['nominal_annual_percentage_rate']} {$currency}</b> TAEG : <b>{$this->oney_response['x3_with_fees']['effective_annual_percentage_rate']} %</b> )</small>
					</div>
                    <div class="payplug-oney-flex">
