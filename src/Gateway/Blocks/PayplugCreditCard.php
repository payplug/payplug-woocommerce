...
class PayplugCreditCard extends PayplugGenericBlock
{
    ...
    public function get_payment_method_data()
    {
        ...
        if ('integrated' === $options['payment_method']) {
            ...
            $data['IP'] = true;
        } elseif ('popup' === $options['payment_method']) {
            $data['popup'] = true;
            $data['payplug_create_intent_payment'] = \WC_AJAX::get_endpoint('payplug_create_intent');
    ...
    {
        $options = get_option('woocommerce_payplug_settings', []);
        if ('integrated' === $options['payment_method'] && !is_wc_endpoint_url('order-pay')) {
            $this->ip_scripts();
        }

        if ('popup' == $options['payment_method']) {
            $this->popup_scripts();
        }
