...
class PayplugOney4xWithoutFees extends PayplugGenericBlock {
	...
	public function check_oney() {
		...
		if ( $this->total_price < $this->gateway->oney_thresholds_min || $this->total_price > $this->gateway->oney_thresholds_max ) {
			$this->description = [
				...
				'class' => 'payment_method_oney_x4_without_fees_disabled'
			];
	...
	public function oney_disabled() {
		...
		if( file_exists(PAYPLUG_GATEWAY_PLUGIN_URL . 'assets/images/checkout/x4_without_fees_' .  $country . '.svg') ){
			$data['icon']['src'] = PAYPLUG_GATEWAY_PLUGIN_URL . 'assets/images/checkout/x4_without_fees_' .  $country . '.svg';
		}else{
			$data['icon']['src'] = PAYPLUG_GATEWAY_PLUGIN_URL . 'assets/images/checkout/x4_without_fees_FR.svg';
		}
