...
class PayplugGateway extends WC_Payment_Gateway_CC
{
	...
	public function __construct()
	{
		...
		add_action('woocommerce_thankyou_payplug', [$this, 'validate_payment']);
	...
	/**
	 * @description Update order and stock in case of error during secure payment authentication
	 *
	 * @param $order
	 * @param $resource
	 * @param $message
	 ...
	 * @return string
	 */
	private function invalid_secure_payment($order = null, $resource = null, $message = '')
	{
		$order_id = PayplugWoocommerceHelper::is_pre_30() ? $order->id : $order->get_id();
		if ($order->get_status() != "failed") {
			$order->update_status('failed', sprintf(__('PayPlug %s OK | Transaction %s failed : %s', 'payplug'), '3DS_AUTHENT', $resource->id, wc_clean($message)));
		}
		...
		// Clean post meta
		update_post_meta($order_id, '_secure_payment', null);
		return '';
	}
	...
	/**
	 * @description Validate order payment when the user is redirected to the success confirmation page.
	 *
	 * @param $order_id
	 * @param $save_request
	 * @param $ipn
	 * @return string|void
	 */
	public function validate_payment($order_id = 0, $save_request = true, $ipn = false)
	{
		...
		// If payment method is Hosted field with 3DS required
		$_secure_payment = get_post_meta($order_id, '_secure_payment');
		$_secure_payment = reset($_secure_payment);
		if (!empty($_secure_payment) && isset($_secure_payment['REQUEST_AUTHENTICATION']) && $_secure_payment['REQUEST_AUTHENTICATION']) {
			$post_form = '';
			foreach (explode('&', $_secure_payment['REDIRECTPOSTPARAMS']) as $params) {
				$param = explode('=', $params);
				$post_form .= '<input type="hidden" name="' . $param[0] . '" value="' . $param[1] . '">';
			}

			$_secure_payment['REQUEST_AUTHENTICATION'] = false;
			update_post_meta($order_id, '_secure_payment', $_secure_payment);

			$redirect_form = '<form id="redirectForm" action="' . $_secure_payment['REDIRECTURL'] . '" method="post" style="display:none;">';
			$redirect_form .= $post_form;
			$redirect_form .= '<script>document.getElementById("redirectForm").submit();</script>';
			$redirect_form .= '</form>';
			echo $redirect_form;
			return;
		}
		...
		if ($payment_method === $this->id) {
			...
			if (!empty($_secure_payment)) {
				$amount = isset($_GET['AMOUNT']) ? (int)$_GET['AMOUNT'] : 0;
				if ($amount != $_secure_payment['AMOUNT']) {
					return $this->invalid_secure_payment($order, $payment, 'Expected amount doesn\'t match');
				}

				$orderid = isset($_GET['ORDERID']) ? (int)$_GET['ORDERID'] : 0;
				if ($orderid != $_secure_payment['ORDERID']) {
					return $this->invalid_secure_payment($order, $payment, 'Expected order id doesn\'t match');
				}
	...
	public function process_standard_payment($order, $amount, $customer_id)
	{
		...
		try {
			...
			//HF VS NORMAL
			if (is_array($payment) && isset($payment['TRANSACTIONID'])) {
				$payment_id = $payment['TRANSACTIONID'];

				// If transaction required 3D secure, save data for authentication usage
				if (isset($payment['EXECCODE']) && 0001 == $payment['EXECCODE']) {
					$payment['REQUEST_AUTHENTICATION'] = true;
					update_post_meta($order_id, '_secure_payment', array_merge($payment, $payment_data['params']));
				}
			} else {
				$payment_id = $payment->id;
				update_post_meta($order_id, '_secure_payment', []);
			}
			...
			$redirect_url = !empty($payment->hosted_payment->payment_url) ? $payment->hosted_payment->payment_url : $return_url;
			$cancel_url = !empty($payment->hosted_payment->cancel_url) ? $payment->hosted_payment->cancel_url : null;
