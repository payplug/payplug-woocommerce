...
class PayplugGateway extends WC_Payment_Gateway_CC
{
    ...
    public function __construct()
    {
        ...
        $payplug_gateways = ['payplug', 'american_express', 'apple_pay', 'bancontact', 'oney_x3_with_fees', 'oney_x3_without_fees', 'oney_x4_with_fees', 'oney_x4_without_fees', 'satispay', 'ideal', 'mybank'];
        ...
        $fields = [
            ...
            'oney_thresholds' => [
                ...
                'description' => sprintf(
                    ...
                    '<b class="min">' . $this->oney_thresholds_min . '</b>',
                    '<b class="max">' . $this->oney_thresholds_max . '</b>'
                ),
                'desc_tip' => false,
            ],
            'oney_thresholds_min' => [
    ...
    public function filter_tokens($tokens, $user_id, $gateway_id)
    {
        ...
        if (false === $this->oneclick) {
            foreach ($tokens as $token_id => $token) {
                if ('payplug' === $token->get_gateway_id()) {
    ...
    public function process_standard_payment($order, $amount, $customer_id)
    {
        ...
        try {
            ...
            if (!empty($is_subscription) && true === $is_subscription) {
    ...
    public function user_logged_in()
    {
        ...
        if (isset($options['client_data'], $options['client_data']['jwt']['test']['access_token'])) {
            return true;
        }
    ...
    public function check_gateway($gateways)
    {
        ...
        if ('with_fees' == $this->oney_type) {
            unset($gateways['oney_x3_without_fees'], $gateways['oney_x4_without_fees']);
        } else {
            unset($gateways['oney_x3_with_fees'], $gateways['oney_x4_with_fees']);
        }
    ...
    public function can_refund_order($order)
    {
        ...
        return $order && $this->supports('refunds') && 'cancelled' !== $status && 'failed' !== $status;
    }
    ...
    public function tmp_generate_jwt()
    {
        ...
        foreach ($client_data as $key => $data) {
            ...
            if (('live' == $key) && isset($data['client_id'], $data['client_secret'])) {
            ...
            if (('test' == $key) && isset($data['client_id'], $data['client_secret'])) {
                $generated_jwt = $this->api->generateJWT($data['client_id'], $data['client_secret']);
    ...
    private function process_standard_intent_payment($order)
    {
        ...
        if (!is_wc_endpoint_url('order-pay')
            ...
            && 'payplug_order_review_url' !== $_GET['wc-ajax']
        ) {
            $order_id = PayplugWoocommerceHelper::is_pre_30() ? $order->id : $order->get_id();
