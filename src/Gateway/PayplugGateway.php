...
class PayplugGateway extends WC_Payment_Gateway_CC
{
    ...
    public $min_oney_price;
    public $oney_thresholds_min;
    ...
    public static function log($message, $level = 'info')
    {
        ...
        PayplugWoocommerceHelper::is_pre_30()
            ...
            : self::$log->log($level, $message, ['source' => 'payplug_gateway']);
    }
    ...
    public function __construct()
    {
        ...
        $payplug_gateways = ['payplug', 'american_express', 'apple_pay', 'bancontact', 'oney_x3_with_fees', 'oney_x3_without_fees', 'oney_x4_with_fees', 'oney_x4_without_fees', 'satispay', 'ideal', 'mybank', 'wero', 'bizum', 'scalapay'];
    ...
    public function validate_payment($id = null, $save_request = true, $ipn = false)
    {
        ...
        if ($payment_method === $this->id) {
            ...
            if (empty($transaction_id)) {
                self::log(sprintf('Order #%s : Missing transaction id.', $order_id), 'error');
    ...
    public function init_form_fields()
    {
        ...
        $fields = [
            ...
            'oney_thresholds' => [
                ...
                'description' => sprintf(
                    __('I would like to offer guaranteed payment in installments for amounts between %s€ and %s€.', 'payplug'),
                    '<b class="min">' . $oney_amount['min'] . '</b>',
                    '<b class="max">' . $oney_amount['max'] . '</b>'
                ),
                'desc_tip' => false,
            ],
        ...
        if ($this->user_logged_in()) {
            ...
            } elseif ('live' === $this->get_current_mode()) {
    ...
    public function admin_options()
    {
        ...
        wp_enqueue_script('app.js', PAYPLUG_GATEWAY_PLUGIN_URL . 'assets/dist/js/app-' . PAYPLUG_GATEWAY_VERSION . '.js', [], PAYPLUG_GATEWAY_VERSION);
        wp_enqueue_style('app.css', PAYPLUG_GATEWAY_PLUGIN_URL . 'assets/dist/css/app-' . PAYPLUG_GATEWAY_VERSION . '.css', [], PAYPLUG_GATEWAY_VERSION);
        wp_localize_script(
            'app.js',
            ...
            [
                ...
                'rest_url' => get_home_url() . '/?rest_route=/payplug_api/',
            ]
        ); ?>
    ...
    {
        self::log(sprintf('Processing payment for order #%s', $order_id));
        ...
        if (is_wp_error($amount)) {
            self::log(sprintf('Invalid amount %s for the order.', $order->get_total()), 'error');
        ...
        if ($payment_token_id && (int) $customer_id > 0) {
            self::log(sprintf('Payment token found.', $amount));
    ...
    private function process_standard_intent_payment($order)
    {
        ...
        if (!is_wc_endpoint_url('order-pay') &&
            ...
            try {
                ...
                self::log(sprintf('Payment intent created for order #%s', $order_id));
                ...
                self::log(sprintf('Error while processing order #%s : %s', $order_id, $e->getMessage()), 'error');
                throw new \Exception(__('Payment processing failed. Please retry.', 'payplug'));
            }
        }
    ...
    public function process_standard_payment($order, $amount, $customer_id)
    {
        ...
        try {
            ...
            self::log(sprintf('Payment creation complete for order #%s', $order_id));
    ...
    public function process_payment_with_token($order, $amount, $customer_id, $token_id)
    {
        ...
        if (!$payment_token || (int) $customer_id !== (int) $payment_token->get_user_id()) {
            self::log('Could not find the payment token or the payment doesn\'t belong to the current user.', 'error');
    ...
    {
        self::log(sprintf('Processing refund for order #%s', $order_id));

        if (!$this->user_logged_in()) {
            self::log(__('You must be logged in with your PayPlug account.', 'payplug'), 'error');
        ...
        $order = wc_get_order($order_id);
        if (!$order instanceof \WC_Order) {
            self::log(sprintf('The order #%s does not exist.', $order_id), 'error');

            return new \WP_Error('process_refund_error', sprintf(__('The order %s does not exist.', 'payplug'), $order_id));
        }

        if ($order->get_status() === 'cancelled') {
            self::log(sprintf('The order #%s cannot be refund.', $order_id), 'error');
        ...
        $transaction_id = PayplugWoocommerceHelper::is_pre_30() ? get_post_meta($order_id, '_transaction_id', true) : $order->get_transaction_id();
        if (empty($transaction_id)) {
            self::log(sprintf('The order #%s does not have PayPlug transaction ID associated with it.', $order_id), 'error');
        ...
        try {
            ...
            self::log('Refund process complete for the order.');

            return true;
        } catch (HttpException $e) {
            self::log(sprintf('Refund request error for the order %s from PayPlug API : %s', $order_id, wc_print_r($e->getErrorObject(), true)), 'error');

            return new \WP_Error('process_refund_error', __('The transaction could not be refunded. Please try again.', 'payplug'));
        } catch (\Exception $e) {
            self::log(sprintf('Refund request error for the order %s : %s', $order_id, wc_clean($e->getMessage())), 'error');
    ...
    public function retrieve_merchant_id($key = null)
    {
        ...
        try {
            ...
            self::log(sprintf('Missing API key for PayPlug client : %s', wc_print_r($e->getMessage(), true)), 'error');
        } catch (HttpException $e) {
            self::log(sprintf('Account request error from PayPlug API : %s', wc_print_r($e->getErrorObject(), true)), 'error');
            ...
            self::log(sprintf('Account request error : %s', wc_clean($e->getMessage())), 'error');
        }
    ...
    public function user_logged_in()
    {
        ...
        if (!empty($jwt) && isset($jwt['test']) && isset($jwt['test']['access_token'])) {
            return true;
        }

        $api_key = json_decode($options['api_key'], true);

        return !empty($api_key) && isset($api_key['test']) && !empty($api_key['test']);
    }
