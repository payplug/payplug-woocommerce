...
class PayplugAddressData
{
	...
	protected function prepare_address_data($order)
	{
		...
		$formated_phone = self::prepare_phone_number($phone, $country);
		if($formated_phone['phone']) {
			if ($formated_phone['is_mmobile']) {
				$this->billing['mobile_phone_number'] = $formated_phone['phone'];
				$this->shipping['mobile_phone_number'] = $formated_phone['phone'];
			} else {
				$this->billing['landline_phone_number'] = $formated_phone['phone'];
			}
		}
	}

	public static function prepare_phone_number($phone = '', $country = '')
	{
		$formated_phone = [
			'phone' => '',
			'is_mobile' => false,
		...
		try {
			...
			$formated_phone = [
				'phone' => $phone_number_util->format($phone_number, PhoneNumberFormat::E164),
				'is_mobile' => PhoneNumberType::MOBILE === $phone_number_util->getNumberType($phone_number),
			];
		} catch (\Exception $e) {
