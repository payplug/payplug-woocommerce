...
class PayplugGatewayOney3x extends PayplugGenericGateway
{
    ...
    public function __construct()
    {
        ...
        if (is_checkout()) {
            if (self::ONEY_DISALBE_CHECKOUT_OPTIONS === $this->check_oney_is_available()) {
                $this->enabled = 'no';
            }
        }
    }
    ...
    public function validate_checkout()
    {
        ...
        if (in_array($posted_data['payment_method'], ['oney_x3_with_fees', 'oney_x4_with_fees', 'oney_x3_without_fees', 'oney_x4_without_fees'])) {
            if (self::ONEY_UNAVAILABLE_CODE_COUNTRY_NOT_ALLOWED === $this->check_oney_is_available()) {
                throw new \Exception(__('Unavailable for the specified country.'));
            } elseif (self::ONEY_UNAVAILABLE_CODE_CART_SIZE_TOO_HIGH === $this->check_oney_is_available()) {
            ...
            } elseif (!$this->check_oney_is_available()) {
    ...
    {
        $disable = '';
        if (true === $this->check_oney_is_available()) {
...
HTML;
    ...
    public function process_standard_payment($order, $amount, $customer_id)
    {
        ...
        try {
            ...
            } elseif (self::ONEY_UNAVAILABLE_CODE_COUNTRY_NOT_ALLOWED === $this->check_oney_is_available()) {
            ...
            if (!filter_var($billing_email, FILTER_VALIDATE_EMAIL) || false !== strpos($billing_email, '+')) {
    ...
    public function check_gateway($gateways)
    {
        if (isset($gateways[$this->id]) && $gateways[$this->id]->id == $this->id) {
            ...
            if (false === $this->check_oney_is_available()) {
    ...
    {
        if ($this->id === $order->get_payment_method() && parent::can_refund_order($order) && 'refunded' !== $order->get_status() && $this->api) {
    ...
    public function checkGateway()
    {
        ...
        if (empty($options) || !isset($options['oney']) || 'no' === $options['oney']) {
            return false;
        }
