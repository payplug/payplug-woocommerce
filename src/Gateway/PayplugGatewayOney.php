...
/**
 * Oney WooCommerce Gateway.
 *
 * @package Payplug\PayplugWoocommerce\Gateway
 */
class PayplugGatewayOney extends PayplugGateway {
	...
	protected $_oney_type;
	protected $logos = [];

	public function __construct() {
		parent::__construct();

		//Regsiter and enqueue css file for oney
		$this->register_oney_style();

		// Show Oney refund text
		add_action( 'woocommerce_order_item_add_action_buttons', [ $this, 'oney_refund_text' ] );

		// Set current option from payplug settings and api call in checkout page
		$this->set_account_data_from_options_if_is_checkout();
	...
	/**
	 * Regsiter and enqueue css file for oney in the settings page "payplug-admin-oney.css"
	 *
	 * @return void
	 */
	public function register_oney_style() {
		if ( is_admin() ) {
	 ...
	 * @return void
	 */
	public function set_account_data_from_options_if_is_checkout() {
		if ( is_checkout() ) {
	...
	/**
	 * Set oney settings (min_amounts, max_amounts and allowed_countries)
	 *
	 * @return void
	 */
	...
	public function get_icon() {
		$is_available = $this->check_oney_is_available();

		if ( $is_available === true ) {
			$total_price         = floatval( WC()->cart->total );
			$this->oney_response = $this->api->simulate_oney_payment( $total_price, $this->_oney_type );
			$currency            = get_woocommerce_currency_symbol( get_option( 'woocommerce_currency' ) );
			if ( is_array( $this->oney_response ) ) {
				$this->description = $this->set_description( $this->oney_response, $currency );
			} else {
				$this->description = $this->oney_response;
			}
		}

		$logo = $is_available ? $this->logos[0] : $this->logos[1];

		$icons = apply_filters( 'payplug_payment_icons', [
			'payplug' => sprintf( '<img src="%s" alt="%s" class="payplug-payment-icon" />', esc_url( PAYPLUG_GATEWAY_PLUGIN_URL . '/assets/images/' . $logo ), $this->id ),
		] );
	...
	public function check_oney_is_available() {
		...
		if ( ! empty( get_query_var( 'order-pay' ) ) ) {
			...
			// get shipping and billing country codes
		...
		// Countries of billing and shipping should be the same and should be on the allowedCountry that comes from pauyplug-api /account
		if ( $country_code_billing != $country_code_shipping || ! in_array( $country_code_billing, $this->allowed_country_codes ) || ! in_array( $country_code_shipping, $this->allowed_country_codes ) ) {
	 ...
	 * @return void
	 */
	private function order_items_to_cart( $cart ) {
		$order = wc_get_order( get_query_var( 'order-pay' ) );
	...
	public function process_standard_payment( $order, $amount, $customer_id ) {
		...
		try {
			// If Oney is not available

			if ( ! $is_available ) {
				throw new \Exception( __( 'Payment processing failed. Please retry.', 'payplug' ) );
			} elseif ( $is_available === self::ONEY_UNAVAILABLE_CODE_COUNTRY_NOT_ALLOWED ) {
				throw new \Exception( __( 'Unavailable for the specified country.' ) );
			} elseif ( $is_available === self::ONEY_UNAVAILABLE_CODE_CART_SIZE_TOO_HIGH ) {
