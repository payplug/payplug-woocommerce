 ...
 */
class Oney4x extends PayplugGatewayOney
{
    ...
    public function __construct()
    {
	    ...
	    $this->logos              = ['lg-4xoney-checkout.png', 'lg-4xoney-checkout-disabled.png'];
    }
		...
		return <<<HTML
                <p>
                    <div class="payplug-oney-flex">
                        ...
                        <div>{$oney_response['x4_with_fees']['down_payment_amount']} {$currency}</div>
                    </div>
                    <div class="payplug-oney-flex">
                        <div>{$f(__('1st monthly payment', 'payplug'))} :</div>
                        <div>{$oney_response['x4_with_fees']['installments'][0]['amount']} {$currency}</div>
                    </div>
                    <div class="payplug-oney-flex">
                        <div>{$f(__('2nd monthly payment', 'payplug'))} :</div>
                        <div>{$oney_response['x4_with_fees']['installments'][1]['amount']} {$currency}</div>
                    </div>
                    <div class="payplug-oney-flex">
                        <div>{$f(__('3rd monthly payment', 'payplug'))} :</div>
                        <div>{$oney_response['x4_with_fees']['installments'][2]['amount']} {$currency}</div>
                    </div>
                </p>
HTML;
	}
