...
class PayplugWoocommerceHelper
{
    ...
    {
        return PayplugGateway::OPTION_NAME . (array_key_exists('mode', $options) && 'yes' === $options['mode'] ? '_live' : '_test');
    }
    ...
    public static function generic_get_account_data_from_options($gateway_id)
    {
        ...
        if (isset($options[$gateway_id]) && 'yes' == $options[$gateway_id]) {
    ...
    public static function set_account_data_from_options()
    {
        ...
        if ('yes' === $options['mode'] && empty($payplug_live_key)) {
            return [];
        }

        if ('yes' != $options['mode'] && empty($payplug_test_key)) {
            return [];
        }

        try {
            $parameters_account = Authentication::getAccount(new Payplug('yes' === $options['mode'] ? $payplug_live_key : $payplug_test_key));
    ...
    public static function is_oney_available()
    {
        ...
        return ($account && '1' == $account['permissions'][PayplugPermissions::USE_ONEY] && 'yes' === $oney_active);
    }
    ...
    {
        $account = self::get_account_data_from_options();
        if ($account && true == $account['permissions'][PayplugPermissions::USE_ONEY] && $account['country'] == self::getISOCountryCode()) {
            return true;
        }
    ...
    public static function UpdateCountryOption($options)
    {
        try {
            ...
            if ((isset($options['mode'])) && $options['payplug_test_key']) {
                $key = 'yes' === $options['mode'] && !empty($options['payplug_live_key']) ? $options['payplug_live_key'] : $options['payplug_test_key'];
            }
    ...
    public static function check_mode()
    {
        ...
        return 'yes' === get_option('woocommerce_payplug_settings', [])['mode'] ? true : false;
    }
    ...
    {
        if (!empty($error_code) && (401 === $error_code || 403 === $error_code)) {
