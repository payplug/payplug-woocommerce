...
(() => {
	...
	if (!!window?.wp?.data?.select('wc/store/cart') &&
		...
		const createCustomElement = () => {
			...
			if (settings?.enabled === true &&
				(cartTotal >= settings?.requirements['min_threshold']) &&
				(cartTotal <= settings?.requirements['max_threshold']) &&
				(qty <= settings?.requirements['max_quantity'])) {
				oney_available = true;
			}
		...
		const renderCustomContent = () => {
			try {
				...
				const shouldDisplayOney = settings?.enabled === true &&
					(cartTotal >= settings?.requirements['min_threshold']) &&
					(cartTotal <= settings?.requirements['max_threshold']) &&
					(qty <= settings?.requirements['max_quantity']);
