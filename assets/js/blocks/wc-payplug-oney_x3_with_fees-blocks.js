 ...
 */
(() => {
  // Only proceed if Oney is enabled in settings
  ...
  if (!!window?.wp?.data?.select('wc/store/cart') && document.body.classList.contains('woocommerce-cart')) {
    // Check if Oney is enabled in settings - if not, exit immediately

    console.log(JSON.stringify(settings));
    if (settings?.enabled !== true) {
      // Clean up any existing container
      const container = document.querySelector('.payplug-totals-container');
      if (container) {
        container.remove();
      }
      return;
    ...
    const createCustomElement = () => {
      ...
      // Only show Oney as available if it's enabled and cart total is within limits
      if (settings?.enabled === true && cartTotal >= settings?.requirements['min_threshold'] && cartTotal <= settings?.requirements['max_threshold'] && qty <= settings?.requirements['max_quantity']) {
        oney_available = true;
      }

      // Only return an element if Oney is available
      if (!oney_available) {
        return null;
      }
    ...
    const renderCustomContent = () => {
      try {
        // Check if Oney is enabled and conditions are met
        const cartStore = select('wc/store/cart');
        let cartTotal = cartStore?.getCartTotals()?.total_price || 0;
        let qty = 0;
        cartStore?.getCartData().items.forEach(item => {
        ...
        // Verify if Oney should be displayed
        const shouldDisplayOney = settings?.enabled === true && cartTotal >= settings?.requirements['min_threshold'] && cartTotal <= settings?.requirements['max_threshold'] && qty <= settings?.requirements['max_quantity'];
        ...
        // If Oney should not be displayed, remove the container if it exists
        if (!shouldDisplayOney) {
          if (container) {
            container.remove();
            root = null;
          }
        ...
        if (cartTotalsBlock) {
          ...
          // Create container if it doesn't exist and Oney should be displayed
          if (!container && shouldDisplayOney) {
    ...
    // Initial setup - only if Oney is enabled
    if (settings?.enabled === true) {
      ...
      // If Oney is not enabled, clean up any existing containers
      const container = document.querySelector('.payplug-totals-container');
      if (container) {
        container.remove();
      }
