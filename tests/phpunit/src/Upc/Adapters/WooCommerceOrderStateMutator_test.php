...
class WooCommerceOrderStateMutator_test extends TestCase
{
     ...
     */
    public function provideStatusesAndTheirTerminalExpectation(): array
    {
        return [
            'processing is terminal' => ['processing', true],
            'completed is terminal' => ['completed', true],
            'refunded is terminal' => ['refunded', true],
            'pending is not terminal' => ['pending', false],
            'on-hold is not terminal' => ['on-hold', false],
            // Deliberately re-payable, per this class's own TERMINAL_STATUSES docblock.
            'failed is not terminal' => ['failed', false],
            'cancelled is not terminal' => ['cancelled', false],
        ];
    }

    /**
     * @dataProvider provideStatusesAndTheirTerminalExpectation
     */
    public function testIsTerminal(string $status, bool $expected): void
    {
        $this->assertSame($expected, WooCommerceOrderStateMutator::isTerminal($status));
    }
}
